...
const qcloudPhoneRegisterStatus = computed(() => Number(authStore.globalConfig.qcloudPhoneRegisterStatus) === 1)
...
<template>
	<div class="px-[65px]" :class="isLogin ? 'pt-[40px]' : 'pt-5'">
		<NForm v-if="!isLogin" ref="formRef" :model="registerForm" :rules="rules" label-placement="left" label-width="auto"
			require-mark-placement="right-hanging" :style="{ maxWidth: '640px' }">
			<Motion :delay="50">
			...
			<Motion :delay="120">
				<NFormItem path="password">
					<NInput v-model:value="registerForm.password" placeholder="请输入您的账户密码" type="password" :maxlength="30"
						show-password-on="click" tabindex="0" @keyup.enter="handlerSubmit" />
				</NFormItem>
			</Motion>
			<Motion :delay="190">
			...
			<NFormItem>
				<NButton v-if="!isSendCaptcha" block type="primary" :disabled="loading" :loading="loading"
					@click="handleSendCaptch">
					发送验证码
				</NButton>

				<div v-else class="flex space-x-2 w-full">
					<NButton block type="primary" :disabled="loading" :loading="loading" class="flex-1" @click="handlerSubmit">
						注册账户
					</NButton>
					<NButton block class="flex-1" :disabled="lastSendPhoneCodeTime > 0" @click="isSendCaptcha = false">
		...
		<!-- login -->
		<NForm v-if="isLogin" ref="formRef" :model="loginForm" size="large" :rules="rules" label-placement="left"
			...
			<Motion :delay="120">
				<NFormItem path="password">
					<NInput v-model:value="loginForm.password" placeholder="请输入您的账户密码" type="password" :maxlength="30"
						show-password-on="click" tabindex="0" @keyup.enter="handlerSubmit" />
				</NFormItem>
			</Motion>
			<NFormItem>
				<NButton block type="primary" :disabled="loading" :loading="loading" class="!mt-[50px]" @click="handlerSubmit">
					登录账户
				</NButton>
			</NFormItem>
		</NForm>
	</div>
	<span v-if="qcloudPhoneRegisterStatus" class="flex justify-center cursor-pointer">
		<NButton text @click="isLogin = !isLogin">{{ logTips }}</NButton>
	</span>

	<div class="flex items-center justify-center space-x-5 " :class="qcloudPhoneRegisterStatus ? 'mt-[16px]' : 'mt-[36px]'">
